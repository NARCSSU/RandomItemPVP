# 数据库系统使用指南

## 📊 功能概述

RandomItemPVP 插件现在支持完整的玩家统计数据系统，可以记录：
- ✅ 胜利/失败次数
- ✅ 击杀/死亡数
- ✅ KD比率
- ✅ 胜率
- ✅ 总游戏场次

## 🗄️ 支持的数据库

### SQLite（默认）
- **优点**：无需额外配置，自动创建本地数据库文件
- **适合**：小型服务器（1-50人）
- **配置**：
```yaml
database:
  type: SQLITE
  sqlite:
    file: 'plugins/RandomItemPVP/data.db'
```

### MySQL
- **优点**：性能强大，支持多服务器共享数据
- **适合**：中大型服务器（50+人）或跨服数据共享
- **配置**：
```yaml
database:
  type: MYSQL
  mysql:
    host: 'localhost'
    port: 3306
    database: 'randomitempvp'
    username: 'root'
    password: 'password'
    properties:
      useSSL: false
      autoReconnect: true
      characterEncoding: 'utf8mb4'
      serverTimezone: 'Asia/Shanghai'
      allowPublicKeyRetrieval: true
    pool:
      maximum-pool-size: 10
      minimum-idle: 2
```

## 📝 MySQL 数据库设置

### 1. 创建数据库
```sql
CREATE DATABASE randomitempvp CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

### 2. 创建用户并授权
```sql
CREATE USER 'ripvp'@'localhost' IDENTIFIED BY '你的密码';
GRANT ALL PRIVILEGES ON randomitempvp.* TO 'ripvp'@'localhost';
FLUSH PRIVILEGES;
```

### 3. 修改配置文件
编辑 `config.yml`，将数据库类型改为 MYSQL 并填写连接信息。

### 4. 重启服务器
插件会自动创建表结构。

## 🎮 游戏内命令

### 查看统计数据
```
/ripvp stats          - 查看自己的统计
/ripvp stats <玩家>   - 查看指定玩家的统计
```

显示内容：
- 胜利/失败次数
- 总场次和胜率
- 击杀/死亡数
- KD比率

### 查看排行榜
```
/ripvp top            - 查看胜利排行榜（默认）
/ripvp top wins       - 查看胜利排行榜
/ripvp top kills      - 查看击杀排行榜
/ripvp top kd         - 查看KD比率排行榜
```

排行榜特性：
- 显示前10名玩家
- 🥇🥈🥉 奖牌显示（前三名）
- KD排行榜需要至少10场游戏才能上榜

## 📊 数据统计规则

### 胜利
- 最后存活的玩家获得胜利

### 失败
- 死亡的玩家记录失败
- 中途退出的玩家记录失败
- 游戏结束时未获胜的玩家记录失败

### 击杀
- 击杀其他玩家时记录

### 死亡
- 被击杀时记录

### 场次统计
- 每次参与游戏并完成（胜利或失败）计为1场

## 🔧 数据库表结构

```sql
CREATE TABLE player_stats (
    uuid VARCHAR(36) PRIMARY KEY,      -- 玩家UUID
    player_name VARCHAR(16) NOT NULL,  -- 玩家名称
    wins INT DEFAULT 0,                -- 胜利次数
    losses INT DEFAULT 0,              -- 失败次数
    kills INT DEFAULT 0,               -- 击杀数
    deaths INT DEFAULT 0,              -- 死亡数
    games_played INT DEFAULT 0,        -- 总场次
    last_played BIGINT DEFAULT 0       -- 最后游玩时间
);
```

## 🚀 性能优化

### SQLite 优化
- 自动使用预编译语句缓存
- 单连接池（SQLite特性）

### MySQL 优化
- HikariCP 高性能连接池
- 预编译语句缓存
- 异步数据库操作（不影响游戏性能）
- 索引优化（wins, kills字段）

## 🔄 数据迁移

### 从 SQLite 迁移到 MySQL

1. 导出SQLite数据：
```bash
sqlite3 plugins/RandomItemPVP/data.db .dump > backup.sql
```

2. 转换SQL语法（SQLite → MySQL）：
- 移除 `BEGIN TRANSACTION;` 和 `COMMIT;`
- 将 `TEXT` 改为 `VARCHAR`
- 将 `INTEGER` 改为 `INT`

3. 导入到MySQL：
```bash
mysql -u ripvp -p randomitempvp < backup.sql
```

4. 修改配置文件为MYSQL类型

5. 重启服务器

## ⚙️ 连接参数说明

| 参数 | 说明 | 推荐值 |
|------|------|--------|
| `useSSL` | 是否使用SSL连接 | `false`（本地数据库） |
| `autoReconnect` | 自动重连 | `true` |
| `characterEncoding` | 字符编码 | `utf8mb4`（支持emoji） |
| `serverTimezone` | 服务器时区 | `Asia/Shanghai` |
| `maximum-pool-size` | 最大连接数 | `10` |
| `minimum-idle` | 最小空闲连接 | `2` |

## 🐛 常见问题

### Q: 数据库连接失败
**A**: 检查以下项目：
1. MySQL服务是否运行
2. 用户名密码是否正确
3. 数据库是否存在
4. 用户是否有权限访问该数据库
5. 防火墙是否允许连接

### Q: 数据不同步
**A**: 
- 所有数据库操作都是异步的，但会立即执行
- 如果出现延迟，可能是数据库负载过高

### Q: 想要重置所有统计数据
**A**: 
```sql
-- SQLite
DELETE FROM player_stats;

-- MySQL
TRUNCATE TABLE player_stats;
```

### Q: 想要删除某个玩家的数据
**A**:
```sql
DELETE FROM player_stats WHERE uuid = '玩家UUID';
```

## 📈 未来计划

- [ ] 更多统计维度（最长连杀、使用最多的物品等）
- [ ] 赛季系统（按月/周重置排行榜）
- [ ] 成就系统
- [ ] 网页统计面板

## 💡 技术细节

### 依赖库
- `sqlite-jdbc 3.45.1.0` - SQLite驱动
- `mysql-connector-j 8.3.0` - MySQL驱动
- `HikariCP 5.1.0` - 连接池

### 异步操作
所有数据库写入操作都是异步的，不会阻塞主线程，确保游戏流畅运行。

### 数据一致性
- 使用UUID作为主键，防止玩家改名导致数据丢失
- 自动更新玩家名称
- 事务支持（MySQL）

