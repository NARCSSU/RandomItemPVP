...
public class GameManager implements Listener {
    ...
    private void destroyArena() {
        ...
        // 清除玩家放置的流体（水和岩浆）
        // 使用游戏区域扫描，只扫描一次，避免重复
        if (spawnLocation != null && !placedFluids.isEmpty()) {
            World world = spawnLocation.getWorld();
            int radius = config.getArenaRadius();
            int centerX = spawnLocation.getBlockX();
            int centerZ = spawnLocation.getBlockZ();
            int minY = Math.max(world.getMinHeight(), spawnLocation.getBlockY() - 64);
            int maxY = Math.min(world.getMaxHeight(), spawnLocation.getBlockY() + 64);
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        if (blockType == Material.WATER || blockType == Material.LAVA) {
            ...
            for (int x = -2; x <= 2; x++) {
                for (int z = -2; z <= 2; z++) {
                    for (int y = -1; y <= 1; y++) {
                        placedFluids.add(blockLoc.clone().add(x, y, z));
                    }
                }
            }
        }
    ...
    public void onBucketEmpty(PlayerBucketEmptyEvent event) {
        ...
        for (int x = -3; x <= 3; x++) {
            for (int z = -3; z <= 3; z++) {
                for (int y = -1; y <= 1; y++) {
                    placedFluids.add(bucketLoc.clone().add(x, y, z));
                }
            }
        }
    }
